...
switch (command) {
  ...
  default:
    ...
    console.log("  init --flow <tpl> [--entry <node>] [--dir <p>]     Init flow state");
    console.log("  route --node <id> --verdict <V> --flow <tpl>       Get next node from graph");
    console.log("  transition --from <n> --to <n> --verdict <V> --flow <tpl> --dir <p>");
    ...
    console.log("  validate-context --flow <tpl> --node <id> [--dir <p>]");
    console.log("                                                     Validate flow-context.json");
    console.log("  finalize [--dir <p>] [--strict]                    Finalize terminal node");
    console.log("  viz --flow <tpl> [--dir <p>] [--json]              Visualize flow graph");
