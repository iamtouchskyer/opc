...
function childFixture(parent, entry = "test-design", flow = "build-verify", label = "child") {
  ...
  child.initReceipt = result;
  return child;
}

test("parent-linked standard init reports only the authoritative parent Mission contract", () => {
  const parent = fixture("brief");
  try {
    const child = childFixture(parent);
    const parentContract = join(parent.session, "mission-contract.json");
    const falseChildContract = join(child.session, "mission-contract.json");

    assert.equal(child.initReceipt.mission_enabled, true);
    assert.equal(existsSync(child.initReceipt.mission_contract), true);
    assert.equal(realpathSync(child.initReceipt.mission_contract), realpathSync(parentContract));
    assert.notEqual(resolve(child.initReceipt.mission_contract), resolve(falseChildContract));
