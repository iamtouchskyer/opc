...
test("parent-linked loop init reports only the authoritative parent Mission contract", () => {
  const parent = fixture("mission-loop-parent-receipt-");
  const child = mkdtempSync(join(tmpdir(), "mission-loop-child-receipt-"));
  try {
    const parentInit = run(parent.dir, ["init-loop", "--mission", parent.missionPath]);
    assert.equal(parentInit.initialized, true, parentInit.errors?.join("\n"));

    writeFileSync(join(child, "acceptance-criteria.md"), criteria());
    writeFileSync(join(child, "plan.md"), plan());
    const childInit = run(child, ["init-loop", "--parent-session", parent.dir]);
    const parentContract = join(parent.dir, "mission-contract.json");
    const falseChildContract = join(child, "mission-contract.json");

    assert.equal(childInit.initialized, true, childInit.errors?.join("\n"));
    assert.equal(childInit.mission_enabled, true);
    assert.equal(existsSync(childInit.mission_contract), true);
    assert.equal(realpathSync(childInit.mission_contract), realpathSync(parentContract));
    assert.notEqual(resolve(childInit.mission_contract), resolve(falseChildContract));
    assert.equal(existsSync(falseChildContract), false);
  } finally {
    cleanup(child);
    cleanup(parent.dir);
  }
