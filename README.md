...
You can also trigger Mission in one natural-language line: `Use Mission Gate:
<task>` maps to `/opc --mission <task>`, and `Use Mission Gate for a long run:
<task>` maps to `/opc loop --mission <task>`. No handwritten Mission Contract
or repeated steering prompt is required.
...
- Codex with native Agent/subagent support (primary host)
- Optional: [Claude Code](https://claude.ai/code) compatibility via
  `node bin/opc.mjs install --host claude`
- Node.js >= 18
- Core runtime has no npm dependencies, no MCP server, no build step
- Claude compatibility only: `opc install-hooks --host claude` installs the
  Node-based auto-flow guard; optional `jq` additionally enables its
...
也可以只写一句自然语言：`用 Mission Gate 做：<任务>` 或 `开启 Mission：<任务>`
等价于 `/opc --mission <任务>`；`用 Mission Gate 长跑：<任务>` 等价于
`/opc loop --mission <任务>`；`重新下注：<观察>` 等价于 `/opc rebet <观察>`。
不需要手写 Mission Contract，也不需要反复粘贴长提示词。
...
- 支持原生 Agent/subagent 的 Codex（主运行宿主）
- 可选：[Claude Code](https://claude.ai/code) 兼容安装：
  `node bin/opc.mjs install --host claude`
- Node.js >= 18
- 核心运行时无 npm 依赖、无 MCP server、无构建步骤
- 仅 Claude 兼容模式需要：`opc install-hooks --host claude` 安装 Node 实现的
  auto-flow guard；可选的 `jq` 额外启用其上下文压缩 hook
