...
2. **Screenshot** — convert HTML to PNG with the active host's installed
   `image-x` helper (Codex primary path shown; Claude compatibility installs use
   the equivalent `~/.claude/skills/` path):
   ```bash
   python3 ~/.codex/skills/image-x/scripts/html2png.py artifacts/output.html --output artifacts/gen.png
   ```
